...
public class PurchaseListServiceImpl implements PurchaseListService{
	...
	@Override
	public List<PurchaseList> list(final PurchaseList purchaseList, Direction direction, String... properties) {
			...
			public Predicate toPredicate(Root<PurchaseList> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if(purchaseList!=null){
					if(StringUtil.isNotEmpty(purchaseList.getPurchaseNumber())){
						predicate.getExpressions().add(cb.like(root.<String>get("purchaseNumber"), "%"+purchaseList.getPurchaseNumber().trim()+"%"));
					}
					...
					if(purchaseList.getbPurchaseDate()!=null){
						predicate.getExpressions().add(cb.greaterThanOrEqualTo(root.<Date>get("purchaseDate"), purchaseList.getbPurchaseDate()));
					}
					if(purchaseList.getePurchaseDate()!=null){
						predicate.getExpressions().add(cb.lessThanOrEqualTo(root.<Date>get("purchaseDate"), purchaseList.getePurchaseDate()));
					}
				}
				return predicate;
			}
