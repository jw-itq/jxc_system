...
public class SaleListServiceImpl implements SaleListService{
	...
	@Override
	public List<SaleList> list(final SaleList saleList, Direction direction, String... properties) {
			...
			public Predicate toPredicate(Root<SaleList> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if(saleList!=null){
					if(StringUtil.isNotEmpty(saleList.getSaleNumber())){
						predicate.getExpressions().add(cb.like(root.<String>get("saleNumber"), "%"+saleList.getSaleNumber().trim()+"%"));
					}
					...
					if(saleList.getbSaleDate()!=null){
						predicate.getExpressions().add(cb.greaterThanOrEqualTo(root.<Date>get("saleDate"), saleList.getbSaleDate()));
					}
					if(saleList.geteSaleDate()!=null){
						predicate.getExpressions().add(cb.lessThanOrEqualTo(root.<Date>get("saleDate"), saleList.geteSaleDate()));
					}
				}
				return predicate;
			}
