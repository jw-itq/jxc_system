...
public class DamageListServiceImpl implements DamageListService{
	...
	public void save(DamageList damageList, List<DamageListGoods> damageListGoodsList) {
		for(DamageListGoods damageListGoods:damageListGoodsList){
			...
			damageListGoods.setDamageList(damageList); // 设置材料报损单
		...
		damageListRepository.save(damageList); // 保存材料报损单
	}

	@Override
	public List<DamageList> list(final DamageList damageList, Direction direction, String... properties) {
			...
			public Predicate toPredicate(Root<DamageList> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if(damageList!=null){
					if(damageList.getbDamageDate()!=null){
						predicate.getExpressions().add(cb.greaterThanOrEqualTo(root.<Date>get("damageDate"), damageList.getbDamageDate()));
					}
					if(damageList.geteDamageDate()!=null){
						predicate.getExpressions().add(cb.lessThanOrEqualTo(root.<Date>get("damageDate"), damageList.geteDamageDate()));
					}
				}
				return predicate;
			}
