...
public class UserServiceImpl implements UserService{
	...
	@Override
	public List<User> list(final User user, Integer page, Integer pageSize, Direction direction, String... properties) {
			...
			public Predicate toPredicate(Root<User> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if(user!=null){
					if(StringUtil.isNotEmpty(user.getUserName())){
						predicate.getExpressions().add(cb.like(root.<String>get("userName"), "%"+user.getUserName()+"%"));
					}
					predicate.getExpressions().add(cb.notEqual(root.get("id"), 1));//排除管理员
				}
				return predicate;
			}
		},pageable);
	...
	@Override
	public Long getCount(final User user) {
