...
public class PurchaseListGoodsServiceImpl implements PurchaseListGoodsService{
	...
	@Override
	public List<PurchaseListGoods> list(final PurchaseListGoods purchaseListGoods) {
			...
			public Predicate toPredicate(Root<PurchaseListGoods> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if(purchaseListGoods!=null){
					...
					if(StringUtil.isNotEmpty(purchaseListGoods.getCodeOrName())){
						predicate.getExpressions().add(cb.or(cb.like(root.<String>get("code"), "%"+purchaseListGoods.getCodeOrName()+"%"), cb.like(root.<String>get("name"), "%"+purchaseListGoods.getCodeOrName()+"%")));
					}
				}
				return predicate;
			}
