...
public class SaleListGoodsServiceImpl implements SaleListGoodsService{
	...
	@Override
	public List<SaleListGoods> list(final SaleListGoods saleListGoods) {
			...
			public Predicate toPredicate(Root<SaleListGoods> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if(saleListGoods!=null){
					...
					if(StringUtil.isNotEmpty(saleListGoods.getCodeOrName())){
						predicate.getExpressions().add(cb.or(cb.like(root.<String>get("code"), "%"+saleListGoods.getCodeOrName()+"%"), cb.like(root.<String>get("name"), "%"+saleListGoods.getCodeOrName()+"%")));
					}
				}
				return predicate;
			}
