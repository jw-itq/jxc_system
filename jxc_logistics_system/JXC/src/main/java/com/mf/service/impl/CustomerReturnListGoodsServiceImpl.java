...
public class CustomerReturnListGoodsServiceImpl implements CustomerReturnListGoodsService{
	...
	@Override
	public List<CustomerReturnListGoods> list(final CustomerReturnListGoods customerReturnListGoods) {
			...
			public Predicate toPredicate(Root<CustomerReturnListGoods> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
				...
				if(customerReturnListGoods!=null){
					...
					if(StringUtil.isNotEmpty(customerReturnListGoods.getCodeOrName())){
						predicate.getExpressions().add(cb.or(cb.like(root.<String>get("code"), "%"+customerReturnListGoods.getCodeOrName()+"%"), cb.like(root.<String>get("name"), "%"+customerReturnListGoods.getCodeOrName()+"%")));
					}
				}
				return predicate;
			}
