...
public class GoodsTypeAdminController {
	...
	public Map<String,Object> save(String name,Integer parentId)throws Exception{
		...
		logService.save(new Log(Log.ADD_ACTION,"添加材料类别信息"));
	...
	public Map<String,Object> delete(Integer id)throws Exception{
		...
		if(goodsService.findByTypeId(id).size()==0){
			...
			logService.save(new Log(Log.DELETE_ACTION,"删除材料类别信息"+goodsType));
			goodsTypeService.delete(id);
			resultMap.put("success", true);
		}else{
			resultMap.put("success", false);
			resultMap.put("errorInfo", "该类别下含有材料，不能删除！");
		}
		return resultMap;
	}
	...
	@RequiresPermissions(value={"材料管理","进货入库","退货出库","销售出库","客户退货","当前库存查询","材料报损","材料报溢","材料采购统计","材料销售统计"},logical=Logical.OR)
	public String loadTreeInfo()throws Exception{
		logService.save(new Log(Log.SEARCH_ACTION,"查询所有材料类别信息"));
